...
simulations = ['/Users/nsha457/Documents/Jupyter_files/SBML_AnnotMerge/Verified_other_conversions/BIOMD0000000001/solutions.csv']
locations=['/Users/nsha457/Documents/Jupyter_files/SBML_AnnotMerge/Verified_other_conversions/BIOMD0000000001/BIOMD0000000001.xml']
...
for address, simulation in zip(locations, simulations):
    ...
    if error:
        ...
        [speciesId, reversibles, irreversibles, reaction_reactants, reaction_products, Synthesis,
         Dtype] = reactionsCategorizing(model_lib, model_simple, compartments, idAndAnnotations)
...
def irreversibleAndReversible(model_lib, model_simple, speciesId, reaction_reactants, reaction_products, solutions,
    ...
    for (reaction, r) in zip(model_simple.getListOfReactionIds(), range(len(model_lib.getListOfReactions()))):
        ...
        if SBase.getSBOTermID(model_lib.getListOfReactions()[r]) == 'SBO:0000079':  # reversible MA
            ...
            for stoichiometry, reactant in reaction_reactants[reaction]:
                phi_Xr_1 = phi_Xr_1 * pow(
                    solutions[[x for x in idAndAnnotations if idAndAnnotations[x] == reactant][0]][1], stoichiometry)
                phi_Xr_e = phi_Xr_e * pow(
                    solutions[[x for x in idAndAnnotations if idAndAnnotations[x] == reactant][0]][
                        len(solutions[[x for x in idAndAnnotations if idAndAnnotations[x] == reactant][0]]) - 1],
                    stoichiometry)

            for stoichiometry, product in reaction_products[reaction]:
                phi_Xp_1 = phi_Xp_1 * pow(
                    solutions[[x for x in idAndAnnotations if idAndAnnotations[x] == product][0]][1], stoichiometry)
                phi_Xp_e = phi_Xp_e * pow(solutions[[x for x in idAndAnnotations if idAndAnnotations[x] == product][0]][
                                              len(solutions[
                                                      [x for x in idAndAnnotations if idAndAnnotations[x] == product][
                                                          0]]) - 1], stoichiometry)
    ...
    for (reaction, r) in zip(model_simple.getListOfReactionIds(), range(len(model_lib.getListOfReactions()))):
        ...
        if SBase.getSBOTermID(model_lib.getListOfReactions()[r]) == 'SBO:0000187':  # irreversible MM
            ...
            popt, pcov = curve_fit(func, solve, unifiedSolution[reaction] / compartmentVolume1, maxfev=300000,
                                   bounds=bounds)
            Vm[reaction] = popt[0]
            Km[reaction] = popt[1]
...
t_span = [0,100]
t, x = bgt.simulate(BGmodel, timespan=t_span, control_vars={}, x0=x_0, dt=0.01)
